...
class EffectDetails(object):
    def __init__(self, detail_string, snp_eff_version = 3.0):
        fields = detail_string.split("|")
        self.effect_name = fields[0].split("(")[0]
        self.effect_severity = fields[0].split("(")[1]
        ...
        ### EFFECT SEVERITY
        self.effect_priority = effect_map[self.effect_name]
    ...
    def __str__(self):
        return "\t".join([self.consequence, self.effect_severity,
                          ...
                          str(self.exon), str(self.so), str(self.effect_priority)])
